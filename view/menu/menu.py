...
class DropDownMenu(DirectObject):
    ...
    def __init__(self, items, parent=None,
        ...
        for t, menuItemsGenerator in items:
            ...
            if underlinePos > -1:
                ...
                self.accept('alt-' + t[underlinePos].lower(), self.__createMenu,
                            [True, menuItemsGenerator, b.getName()])
    ...
    def __createMenu(self, clicked, menuItemsGenerator, DBname, crap=None):
        ...
        if self.edgePos == DropDownMenu.PLeft:
            ...
            x = fr[1] - self.dropDownMenu.maxWidth if self.alignment == DropDownMenu.ARight else fr[0]
            z = fr[3] - (self.height - self.gapFromEdge) / self.scale[2] if self.edgePos == DropDownMenu.PTop else fr[
                                                                                                                       2] + (
                                                                                                                       self.height) / \
                                                                                                                   self.scale[
                                                                                                                       2] + self.dropDownMenu.height
...
class PopupMenu(DirectObject):
    ...
    def __init__(self, items, parent=None, buttonThrower=None, onDestroy=None,
            ...
            if item:
                ...
                if isinstance(imgPath, bool):
                    ...
                    if img is not None:
                        ...
                        CM = CardMaker('')
                        CM.setFrame(-2 * imageHalfHeight - leftPad, -leftPad, itemZcenter - imageHalfHeight,
                ...
                if underlinePos > -1:
                    ...
                    if disabled:
                        ...
                        underline.copyTo(b.stateNodePath[1], 10).setColor(
                            Vec4(*textColorHover if haveSubmenu else textColorPress), 1)
                        underline.copyTo(b.stateNodePath[2], 10).setColor(Vec4(*textColorHover), 1)
        ...
        try:
            ...
            minZ = self.menu.getChild(0).getRelativePoint(self.menu, Point3(0, 0, b.getTightBounds()[0][2]))[
                       2] - baselineToBot * .5
        try:
            top = self.menu.getChild(0).node().getFrame()[3]
        except:
            top = self.menu.getChild(0).getZ() + baselineToTop
        l, r, b, t = -leftPad - bgPad / self.scale[0], maxWidth + bgPad / self.scale[0], minZ - bgPad / self.scale[
        ...
        z = clamp(minZ + menuBorderDims[2] * self.scale[2] * self.parent.getSz(render2d), .98,
